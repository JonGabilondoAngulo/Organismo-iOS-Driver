...
- (void)motionRefresh:(id)sender
{
    ...
    //NSLog(@"%@", self.motionManager.deviceMotion.attitude.quaternion);
...
- (void)startGravity
{
    if ( ! [self isGravityActive]) {
        ...
        if ([self.motionManager respondsToSelector:@selector(deviceMotion)]) {
            //NSLog(@"YES");
        }
        if ([self.motionManager respondsToSelector:@selector(isGyroActive)]) {
            //NSLog(@"YES");
        }
        if ([CMMotionManager respondsToSelector:@selector(availableAttitudeReferenceFrames)]) {
            //NSLog(@"YES");
        }
        if ([[CMMotionManager class] respondsToSelector:@selector(sharedManager)]) {
            //NSLog(@"YES");
        }
